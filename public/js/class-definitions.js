 ...
 */
 ...
 function favoritePlanet(currentPlanet){
  if (planets.indexOf(currentPlanet) > -1){
    var randomPlanet = planets[Math.floor(Math.random()*(planets.length-0) + 0)];
...
var notWarmOrColdAnimal = new Animal ('Llama', 'male').isWarmBlooded();
...
var shortStory = new House(1).isATallStory(5);
 ...
 *
 */
var kitchenLightsOn = new Lightbulb('off').flipSwitch('on');
var porchLightsOff = new Lightbulb('off').flipSwitch('off');
...
var cookieMonsterBlocked = new Cookie('raisin').swipedByCookieMonster('Friday');
